...
public class PersonDAOImpl implements PersonDAO {
	...
	public List<Person> findSameCourse(Long courseId) {
		...
		List<Person> result = session.createQuery(hql).setParameter("courseId", courseId).list();
		return result;
	}
	...
	public Person getPersonByFirstName(String firstName) {
		...
		NativeQuery query = session.createSQLQuery(sql).addEntity(Person.class).setParameter("firstName", firstName);
	...
	public Person getPersonByLastName(String lastName) {
		...
		NativeQuery query = session.createSQLQuery(sql).addEntity(Person.class).setParameter("lastName", lastName);
		List list = query.list();
		if (list != null && list.size() > 0) {
			return (Person) query.list().get(0);
		} else {
	...
	public boolean checkPersonInDB(Person person) {
		...
		NativeQuery query = session.createSQLQuery(sql).addEntity(Person.class).setParameter("id", person.getId());
		List list = query.list();
		if (list != null && list.size() > 0) {
			return true;
		} else {
